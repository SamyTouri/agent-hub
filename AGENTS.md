...
| **What we know about the market** (started 2026-08-01) | `market-intel/` — measurements we made ourselves, the actors, the rules of the game, and the open questions. **Start at `market-intel/index.md`**: it carries a routing table saying which single page answers which question, so read one page rather than the folder. Conventions and the MESURÉ / RAPPORTÉ / DÉCLARÉ rule are in `market-intel/README.md`. Written in French, Obsidian-compatible wikilinks, every page dated and correctable in place |
